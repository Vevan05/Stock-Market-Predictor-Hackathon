...
function App() {
  ...
  const handlePredict = async () => {
    try {
      ...
      const futurePrices = [];
      let today = new Date();
      for (let i = 0; i <= years * 365; i += 30) {
        futureDates.push(new Date(today.getTime() + i * 24 * 60 * 60 * 1000).toLocaleDateString());
        futurePrices.push(predicted_price);  // Example: flat prediction, could be updated
      }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.heading}>Stock Price Prediction</Text>
      </TouchableOpacity>
      ...
      <View style = {{flex: 1, flexDirection: 'row', justifyContent: 'center'}}>
        <View>
        <div 
        style = {{width: 600, height: 533}}>
        <View>
        <View>
          <img src = 'https://cdn.pixabay.com/photo/2024/01/06/02/44/ai-generated-8490532_640.png' style = {{width: 600, height: 400, marginLeft: 20, marginTop: 30}}/></View>
        <View style = {{flex: 1, flexDirection: 'row',  marginTop: 10, marginLeft: 50}}>
        <Text style = {{alignSelf: 'center', fontFamily: 'Verdana', fontSize: 28}}>Enter Stock Symbol: </Text>
        <TextInput
          style={styles.input}
          value={stockSymbol}
        ...
        <TouchableOpacity style = {{height: 50, justifyContent: 'center', borderRadius: 5, borderWidth: 2, borderColor: '#6A9C89', backgroundColor: '#C4DAD2'}}>
          <Text style = {{alignSelf: 'center', fontFamily: 'Verdana', fontSize: 28, borderColor: '#6A9C89'}}> Predict</Text>
          </TouchableOpacity>
        </View>
      </View>
  ...
  <Text style = {{padding: 5, textAlign: 'center'}}>Upstox</Text>
    ...
    <View>
        <Text style={styles.noPrediction}>Prediction</Text>
